...
export async function GET(request: Request) {
  ...
  try {
    ...
    let url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&page=${page}&sort_by=${sortBy}&language=en-US&with_original_language=en`;
    
    // BLOCK ROMANCE GENRE COMPLETELY
    if (genre && genre === '10749') {
      return NextResponse.json({
        page: 1,
        results: [],
        total_pages: 0,
        total_results: 0,
        content_filter: 'romance_genre_blocked',
        message: 'Romance content is not available'
      }, {
        headers: {
    ...
    // Exclude adult content at API level
    ...
    return NextResponse.json(
      { error: 'Failed to fetch discover movies', details: error },
      { status: 500 }
    );
  }
}